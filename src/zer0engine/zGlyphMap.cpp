...
void GlyphMap::bindMap()
{
	glBindTexture(GL_TEXTURE_2D, _map);
}

int GlyphMap::renderMap(FT_Face face, float pt_size, unsigned int DPI)
{
	_size = pt_size;
	_pointSize = true;
	int map_w, map_h;
...
int GlyphMap::renderMap(FT_Face face, int px_size)
{
...
unsigned char* GlyphMap::renderMapPixels(FT_Face face, int px_size, int & map_w, int & map_h)
{
	_size = static_cast<float>(px_size);
	_pointSize = false;
	GLubyte* pixels =  renderGlyphMap(face, 0, map_w, map_h);
...
unsigned char* GlyphMap::renderGlyphMap(FT_Face face, int DPI, int & map_w, int & map_h)
{
	//free old map
	freeMap();
	...
	for(int i = 0; i < Z_GLYPH_MAP_NUM_CHARACTERS; i++)
	{
		...
		{
			memset((_glyphs+i), 0, sizeof(Glyph));
			ERROR("While glyph rendering at ascii character '%c' (%d)\n", static_cast<unsigned char>(i), i);
			continue;
		}
...
void GlyphMap::createTexture(GLubyte * pixels, int map_w, int map_h)
{
	...
	if(map_w > max_tex_size || map_h > max_tex_size){
		WARNING("GlyphMap: Maximum supported texture size of %dx%d exceeded! GlyphMap requires texture with dimensions %dx%d!",
					max_tex_size, max_tex_size, map_w, map_h);
		return;
	}
	...
	if(CONFIG.SUPPORTS_NEW_GL){// if newer OpenGL version is supported store texture as single R component
		internal_format = GL_RED;
	}
	glTexImage2D(GL_TEXTURE_2D, 0, internal_format, map_w, map_h, 0, GL_RED, GL_UNSIGNED_BYTE, pixels);
...
int GlyphMap::getTextBuffer(const char * text, BufferGlyph * buffer, int buffer_size, Rect * rect)
{
	int count = 0;
	Rect b;
	float cursorX = 0.f;
	...
	while(*text != '\0' && count < buffer_size)
	{
		...
		if((char)c != '\n' && _glyphs[c].width * _glyphs[c].height > 0)//glyph is actually visible
		{
			...
			g->v5[0]= originX	+ _glyphs[c].width;
			g->v5[1] = originY;
			g->t5[0] = (mapX	+ _glyphs[c].width)/mapW; 
			g->t5[1] = (mapY					  )/mapH; 
			count++;
		}
		//shift cursor
